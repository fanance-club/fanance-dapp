...
contract FANCTokenICO is Ownable {
	...
	modifier isNotTradedOnExchange() {
		require(!token.tradedOnFANCExchange(), "FANC Token is traded on exchange");
		_;
	}
	...
	function invest(uint256 _amount) public isNotTradedOnExchange returns (bool) {
		...
		WETHToken(WETHTokenAddress).transferFrom(msg.sender, ICOTreasury, _amount);

		numberOfInvestorsInCurrentRound = numberOfInvestorsInCurrentRound.add(1);
